...
#
# Tags look like `v<Vikunja version>-<release>`, which is what this repository
# has always used (`v2.5.0-0`, `v2.4.0-0`, …):
#
# - if defaults/main.yml points at a Vikunja version that has never been
#   released, the release counter restarts at 0 (`v2.6.0-0`)
# - otherwise the counter is incremented (`v2.5.0-1`), but only if something
...
# Anchored on `vikunja_version:` so that neither
# `vikunja_container_image_self_build_repo_version:` nor a commented-out line
# can be mistaken for it.
version="$(sed -nE 's|^vikunja_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Vikunja version from $defaults_path"
	exit 1
fi

# The version value in defaults/main.yml carries no leading `v` (e.g. `2.5.0`)
# while the tags do. Stripping first keeps this correct either way.
